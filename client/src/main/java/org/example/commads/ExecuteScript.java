...
public class ExecuteScript extends Command {
            ...
            if (st.length == 1) {
                ...
                if (i > -1 && m - i <= 9) {
                    ...
                    try {
                        ...
                        m = i+10;
                        ResponseSender.sendCommandWithWorker("add", worker);
                    } catch (NumberFormatException e) {
                        OutputHandler.println("В файле есть некорректные данные");
                        break;
                    }
                } else {
                    ResponseSender.sendCommand(st[0]);
                }
            } else if (st.length == 2) {
                if (st[0].equals("execute_script")) {
                    ...
                    ResponseSender.sendCommandWithArg(st[0], st[1]);
                }
            }
    ...
    private boolean updateForScript(String id, ArrayList<String> listCommands, int i) {
        ResponseSender.sendCommandWithArg("update_by_id", id);
        Worker worker;
        ...
        if (flag) {
            ...
            ResponseSender.sendCommandWithWorker("update_by_id", worker);
            OutputHandler.println(RequestReader.read());
            return true;
        } else {
